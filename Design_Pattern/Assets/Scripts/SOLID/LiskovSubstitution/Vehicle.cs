...
public class Vehicle : MonoBehaviour 
{
    ...
    */
    ...
    /*
        ...
        — 추상화를 단순하게 유지합니다. 기본 클래스에 들어가는 로직이 많을수록 LSP를 위반할 
        확률도 커집니다. 기본 클래스는 파생 서브 클래스의 일반적인 기능만 표현해야 합니다.
        — 서브 클래스는 기본 클래스와 동일한 공용 멤버를 가져야 합니다. 또한 그러한 공용 
        멤버는 호출 시 동일한 서명을 갖고 동일한 동작을 취해야 합니다.
        — 클래스 계층 구조를 수립할 때 클래스 API를 고려합니다.
        대상을 모두 차량으로 간주하더라도 Car와 Train은 각각 서로 다른 부모 클래스로부터 
        상속하는 편이 더 나을 수 있습니다. 실질적으로 분류가 항상 클래스 계층 구조와 
        일치하지는 않습니다.
        — 상속보다는 합성을 우선시합니다. 상속을 통해 기능의 전달을 시도하는 대신, 특정한 
        동작을 캡슐화할 있도록 인터페이스 또는 별도의 클래스를 만드세요. 그런 다음 믹스 
        앤 매치를 통해 다양한 기능의 합성물을 생성합니다. 
    */
}
